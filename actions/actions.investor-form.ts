...
export async function submitInvestorByCompanyName(
  ...
  try {
    ...
    if (!companyId) {
      return {
        success: false,
        error: "No company provided and unable to create one",
      };
    }
    ...
    let createdFounderContactId: string | null = null;
    let createdInvestorContactId: string | null = null;
    ...
    try {
      ...
      const investorData = {
        ...
        previous_raised: toNumeric(formData.previousRaised),
        paid_percentage: formData.paidPercentage || null,
        investor_types: formData.investorTypes || null,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      };
      ...
      if (formData.id) {
        ...
        if (error) {
          return {
            success: false,
            error: `Failed to update investor: ${error.message}`,
          };
        }
        result = data;
      } else {
        ...
        if (error) {
          return {
            success: false,
            error: `Failed to add investor: ${error.message}`,
          };
        }
        result = data;
      }
      ...
      if (!formData.id && formData.email) {
        ...
        const investmentPreference =
          formData.type === "individual" || formData.type === "angel"
            ? "individual"
            : "business";
        ...
        if (!existingProfile?.id) {
          ...
          const { data: newProfile, error: profileError } = await supabase
            ...
            .insert({
              ...
              investor_category: formData.type,
          ...
          if (profileError) {
            throw new Error(
              `Failed to create investor profile: ${profileError.message}`
            );
          }
        ...
        if (formData.company) {
          ...
          if (existingFirm?.id) {
            ...
            if (firmError) {
              throw new Error(
                `Failed to create investor firm: ${firmError.message}`
              );
            }
            ...
            if (contactErr) {
              throw new Error(
                `Failed to create the investor contacts ${contactErr.message}`
              );
            }
            createdInvestorContactId = contact.id;
          }
        }

        if (userId && createdFirmId) {
          const { data: existingContact } = await supabase
            .from("founder_contacts")
            .select("id")
            .eq("founder_id", userId)
            .eq("investor_firm_id", createdFirmId)
            .maybeSingle();
          ...
          if (!existingContact?.id) {
            ...
            if (contactError) {
              throw new Error(
                `Failed to create founder contact: ${contactError.message}`
              );
            }
            createdFounderContactId = newContact.id;
          } else {
            createdFounderContactId = existingContact.id;
          }
        }
      }
      ...
      if (!formData.id) {
        ...
        if (createdInvestorContactId) {
          await supabase
            .from("investor_contacts")
            .delete()
            .eq("id", createdInvestorContactId);
        }

        if (createdFounderContactId) {
          await supabase
            .from("founder_contacts")
            .delete()
            .eq("id", createdFounderContactId);
        }
      }
      throw innerError;
    }
