...
import { InvestorDetailSheet } from "@/components/admin/investor-detail-sheet";
import {
    Upload,
    AlertCircle,
    Trash2,
...
export default function InvestorsUploadPage() {
    const { investors, error, errors, duplicates, handleFileDrop, clearInvestors, deleteInvestors, saveInvestors } =
        useInvestorUpload();
    ...
    const [selectedInvestor, setSelectedInvestor] = useState<InvestorCsvData | null>(null);
    const [sheetOpen, setSheetOpen] = useState(false);
    const router = useRouter();

    // Detect source from the uploaded data with proper typing using utility
    const detectedSource = investors.length > 0 ? investors[0].source : "admin";
    const sourceStats = investors.reduce((acc, inv) => {
        const source = inv.source;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.log("📊 Detected source distribution:", sourceStats);

            const savedData = await saveInvestors(); // No need to pass source, it's auto-detected

            console.log("=== SAVE OPERATION COMPLETED ===");
            console.log("Saved investors:", savedData.length);
    ...
    const handleViewDetails = (investor: InvestorCsvData) => {
        ...
        <>
            <div className="flex flex-col gap-6 p-4 sm:p-6 w-full max-w-full">
                ...
                <div className="flex flex-col gap-2">
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Upload Investor Data</h1>
                    <p className="text-muted-foreground">Import investor data from CSV or Excel files</p>
                </div>
              ...
              `}
                            ...
                            {isUploading ? (
                                <>
                                    ...
                                    <p className="text-sm text-muted-foreground mt-2">
                                        Parsing and validating investor data
                                    </p>
                                </>
                            ) : uploadSuccess ? (
                                <>
                    ...
                    `}
                                    ...
                                    <p className="text-sm text-muted-foreground/70 mt-2">
                                        Supports .csv and .xlsx files with investor data
                                    </p>
                                </>
                            )}
                        </div>
                        ...
                        {saveSuccess && (
                            <Alert className="border-green-500/20 bg-green-500/5 text-green-400 animate-in slide-in-from-top-2 duration-300">
                                ...
                                <AlertDescription>
                                    ...
                                    <div className="text-sm">All investor firms have been saved to the database.</div>
                                </AlertDescription>
                            </Alert>
                        )}
                                ...
                                <AlertDescription>
                                    ...
                                    <div className="text-sm">
                                        {Object.keys(sourceStats).length === 1 ? (
                                            <>
                                                All {investors.length} records will be saved as{" "}
                                                <Badge variant="outline" className="mx-1 text-xs">
                                                    {InvestorSourceUtils.getSourceDescription(detectedSource)}
                                                </Badge>
                                            ...
                                            <>
                                                ...
                                                {Object.entries(sourceStats).map(([source, count], index) => (
                                                    <span key={source}>
                                                        {index > 0 && ", "}
                                                        <Badge variant="outline" className="mx-1 text-xs">
                                                            {InvestorSourceUtils.getSourceDescription(source)} ({count})
                                                        </Badge>
                                                    </span>
                                                ))}
                                            </>
                        ...
                        <CardHeader className="pb-4">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <Badge variant="outline">{investors.length} records</Badge>
                                </div>
                            </div>
                        </CardHeader>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow className="bg-muted/50">
                                                ...
                                                <TableHead className="min-w-[100px] font-semibold">Source</TableHead>
                                        ...
                                        <TableBody>
                                            {investors.map((investor, index) => {
                                                ...
                                                const investorSource = investor.source;
                                                return (
                                                    <TableRow
                                                        key={investor.id}
                                                        className={`
                    ...
                    `}
                                                    >
                                                        ...
                                                        <TableCell>
                                                            <Badge
                                                                variant="outline"
                                                                className={`text-xs ${getInvestorTypeColor(
                                                        ...
                                                        <TableCell>
                                                            <div className="text-sm">{investor.hq_location || "—"}</div>
                                                        </TableCell>
                                                        <TableCell>
                                                        ...
                                                        <TableCell>
                                                            <div className="text-sm">
                                                                {formatArrayValue(investor.stage_focus)}
                                                            </div>
                                                        </TableCell>
                                                        <TableCell>
                                                            <Badge
                                                                variant="outline"
                                                                className={`text-xs ${getActivityScoreColor(
                                                        ...
                                                        <TableCell>
                                                            <Badge
                                                                variant="outline"
                                                                className={`text-xs ${InvestorSourceUtils.getSourceBadgeColor(
                                                                    investorSource
                                                                )}`}
                                                            >
                                                                {InvestorSourceUtils.getSourceDescription(
                                                                    investorSource
                                                                )}
                                                            </Badge>
                                                        </TableCell>
                ...
                {investors.length === 0 && !error && !isUploading && (
                    <Card className="text-center py-12">
                        <CardContent>
                            ...
                            <h3 className="text-xl font-semibold mb-3">Ready to Import Investor Data</h3>
                            <p className="text-muted-foreground max-w-md mx-auto mb-6">
                                Upload a CSV or Excel file containing investor information to get started. We'll
                                validate the data and show you a preview before saving.
                            </p>
                            <div className="text-sm text-muted-foreground">
                                <p className="mb-2">Required fields include:</p>
                                <div className="flex flex-wrap justify-center gap-2 mb-4">
                                    {["Firm Name", "Website URL", "LinkedIn URL", "Investor Type", "HQ Location"].map(
                                        (field) => (
                                ...
                                <p className="text-xs text-muted-foreground">
                                    💡 <strong>Tip:</strong> Include a "source" column in your file to specify data
                                    origin (admin, ai, etc.). If not provided, data will be marked as "admin" by
                                    default.
                                </p>
                            </div>
                        </CardContent>
            ...
            {/* Detail Sheet */}
            <InvestorDetailSheet investor={selectedInvestor} open={sheetOpen} onOpenChange={setSheetOpen} />
        </>
    );
}
